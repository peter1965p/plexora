...
export const STANDARD_JOB_HTML = `<div style="min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px">
  <div style="max-width:680px;width:100%">
    ...
    <div style="text-align:center;margin-bottom:40px">
      {{#if campaign.logoUrl}}
        <div style="margin-bottom:12px"><img src="{{campaign.logoUrl}}" style="max-height:60px;max-width:200px;object-fit:contain" alt="Logo" /></div>
      {{else}}
        ...
        {{#if campaign.hasCompanyName}}
          <div style="font-size:20px;font-weight:700;color:var(--text-primary)">{{campaign.companyName}}</div>
        {{/if}}
      {{/if}}
